...
export default function Report() {
  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Card className="p-4 sm:p-5"><p>Ambiental</p><b className="text-2xl sm:text-3xl">{Math.round(data?.environmentalScore || 0)}</b></Card><Card className="p-4 sm:p-5"><p>Social</p><b className="text-2xl sm:text-3xl">{Math.round(data?.socialScore || 0)}</b></Card><Card className="p-4 sm:p-5"><p>Governança</p><b className="text-2xl sm:text-3xl">{Math.round(data?.governanceScore || 0)}</b></Card><Card className="p-4 sm:p-5"><p>Geral</p><b className="text-2xl sm:text-3xl">{Math.round(data?.generalScore || 0)}</b><p className="badge mt-2">{labelStatus(data?.classification)}</p></Card></div>
